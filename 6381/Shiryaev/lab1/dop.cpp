...
Field primearrangement(Field field, int i, int j){
	//cout << "in it";
	//cout << field.getnumber() << endl;
	for(int n = ((field.getsize()/2) - 1); n > 0; n--){
		...
		if(shape.mayInsert(i,j,n)){
			...
			//j++;
			if(shape.IsThereFree(i,j)){
				shape = primearrangement(shape, shape.FindFreeStr(i,j), shape.FindFreeCol(i,j));
				//shape.printField();
			}
			else if(bestNum >= shape.getnumber()){
				bestShape = shape;
...
int main(){
	...
	if(A.geteven()){
		...
		A = primearrangement(A, A.FindFreeStr(0,0), A.FindFreeCol(0,0));
