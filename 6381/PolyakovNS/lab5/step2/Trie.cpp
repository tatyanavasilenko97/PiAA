...
void item::print(){
    ...
    //std::cout << number[0].first << " " << number[0].second <<
   std::cout << std::endl;
...
std::vector<std::pair<int,int> > Trie::AK(const std::string& str){
    ...
    for(int i = 0 ; i < str.size() ; ++i){
        ...
        //std::cout << i <<") " << cur->name << " ref: " << cur->ref->name << " size: " << cur->number.size() << std::endl;
        
        item *tmp1 = cur;
        while(tmp1 != tmp1->ref){
            //std::cout << " tmp1 name: " << tmp1->name << " tmp1 size: " << tmp1->number.size() << std::endl;
            if(tmp1->number.size() != 0){
                for(int j = 0 ; j < tmp1->number.size() ; ++j){
                    result.push_back(std::pair<int,int>(i - tmp1->number[j].second + 1,tmp1->number[j].first));
                    //std::cout << i << ") " << cur->name << std::endl;
                }
            }
            tmp1 = tmp1->ref;
        }
        /*if(cur->number.size() != 0){
            item *tmp = cur;
            while(tmp != tmp->ref){
                for(int j = 0 ; j < tmp->number.size() ; ++j){
                    result.push_back(std::pair<int,int>(i - tmp->number[j].second + 1,tmp->number[j].first));
                    std::cout << i << ") " << cur->name << std::endl;
                }
...
void Trie::addString(int number, const std::string& str){
    ...
    cur = cur->addItem(str[k]);
    cur->number.push_back(std::pair<int,int>(number,str.size()));
}
